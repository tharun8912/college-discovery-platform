...
function Cell({
  ...
  return (
    <td
      className={`p-4 align-top text-sm text-slate-700 ${
        isBest
...
export default function CompareTable({
  ...
  const bestPlacement = getBestCollegeIds(
    colleges,
    "placement"
  );
  const bestRating = getBestCollegeIds(
    colleges,
    "rating"
  );
  const bestCourses = getBestCollegeIds(
    colleges,
    "courseCount"
  );
  ...
  const metrics = [
    {
      ...
      bestIds: new Set<number>(),
      render: (c: College) => c.location ?? "N/A",
    },
    {
      key: "fees",
      label: "Annual fees",
        ...
        <table className="w-full min-w-[640px] border-collapse text-left">
          <thead>
            <tr className="border-b border-slate-200">
              <th className="sticky left-0 top-0 z-30 min-w-[140px] border-b border-slate-200 bg-[#f5f7fa] p-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                Criteria
              </th>
          ...
          <tbody>
            {metrics.map((row) => (
              ...
              >
                <th
                  scope="row"
                  className="sticky left-0 z-10 bg-white p-4 text-sm font-medium text-slate-600"
                >
                ...
                {colleges.length < 3 && (
                  <td className="bg-slate-50/50 p-4 text-sm text-slate-300">
                    —
                  </td>
                )}
              </tr>
